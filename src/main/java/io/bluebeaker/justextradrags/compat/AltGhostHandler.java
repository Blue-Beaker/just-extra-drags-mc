...
public class AltGhostHandler<T extends GuiContainer> extends GenericGhostHandler<T> {
    ...
    public <I extends Slot> AltGhostHandler(Class<I> applySlot,boolean checkFit) {
        super(applySlot);
        this.checkFit = checkFit;
    }

    public <I extends Slot> AltGhostHandler(Class<I> applySlot, boolean checkFit, Collection<Integer> slotIDs) {
        super(applySlot);
        this.checkFit = checkFit;
    ...
    public String toString(){
        return this.getClass().getName()+":"+this.applySlot.getName()+"@ slotID"+slotIDs.stream().map(String::valueOf).collect(Collectors.joining(","))+(this.checkFit ?"":" (Unchecked)");
    }
}
